...
if (isset($_GET['add_to_cart'])) {
    $product_id = 'hp_' . $_GET['add_to_cart'];
  
    if (isset($_SESSION['cart'][$product_id])) {
        $_SESSION['cart'][$product_id]['quantity'] += 1;
    } else {
        $raw_id = str_replace('hp_', '', $product_id);
        $stmt = $conn->prepare("SELECT * FROM hp_products WHERE id = ?");
    ...
    if (isset($_GET['buy_now']) && $_GET['buy_now'] == 1) {
        ...
        header("Location: hp.php"); // Add to cart → stay on home
    }
    exit();
  }
?>
...
<div class="row px-1">
    <div class="col-md-10 px-3">
        <div class="row px-4 py-4">
            <?php
            $query = "SELECT * FROM hp_products WHERE brand = 'Hp'";
            ...
            <div class="card h-100 product-card <?php if ($highlight_id == $row['id']) echo 'border border-warning border-3'; ?>">
                    ...
                    <div class="card-body">
                        ...
                        <a href="hp.php?add_to_cart=<?php echo $row['id']; ?>" class="btn btn-info">Add to Cart</a>
                        <a href="hp.php?add_to_cart=<?php echo $row['id']; ?>&buy_now=1" class="btn btn-success">Buy Now</a>
                    </div>
                </div>
            </div>
            <?php endwhile; ?>
